...
class ClientWindow(QMainWindow):
    ...
    def showEvent(self, *args, **kwargs):
        ...
        # подписки
        self.commonChatThread = consumingThread(self.commonMessages_callback,
                                                'to_client', 'msg.common')
        self.privateChatThread = consumingThread(self.privateMessages_callback,
    ...
    def commonMessages_callback(self, ch, method, props, body):
        if props.headers.get('sender') == None:
            return

        time = datetime.strftime(datetime.now(), "[%H:%M:%S]")
        common_edit = self.TabChats.widget(0)
        common_edit.append('<b>' + props.headers.get('sender') + ' ' + time + '</b>: ' + body.decode('utf-8'))
    ...
    def privateMessages_callback(self, ch, method, props, body):
        sender = props.headers.get('sender')
        if sender == None:
            return

        index = None
        for i in range(1, self.TabChats.count()):
            if self.TabChats.tabText(i) == sender:
                index = i
                break

        if index == None:
            # TODO создание новой вкладки для отправителя
            index = self.TabChats.count() - 1
            pass

        print('PRIVATE', props.headers.get('sender'), ': ', body.decode('utf-8'))
