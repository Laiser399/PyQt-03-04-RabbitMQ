...
class ClientWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        channel.basic_publish(exchange='to_server', routing_key='get_online_users',
                                   body='')
        connection.close()
    ...
    def setMenu(self):
        ...
        mFile.addAction('&Выйти', self.close)
    ...
    def closeEvent(self, *args, **kwargs):
        ...
        connection = pika.BlockingConnection(pika.ConnectionParameters(
            host='localhost', port=5672))
        channel = connection.channel()
        channel.basic_publish(exchange='to_server', routing_key='logout',
                                   body=self.nickname)
        connection.close()
    ...
    def slot_sendMessage(self):
        ...
        if self.TabChats.currentIndex() == 0:
            channel.basic_publish(exchange='to_server', routing_key='msg',
    ...
    def slot_author(self):
        QMessageBox.information(None, 'Автор',
                                'Студент МАИ\n'
                                'Группа: М8О-213Б-17\n'
